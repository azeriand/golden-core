        ...
        <section className='grid grid-cols-2 gap-2 grid-flow-row'>
            <div className='flex flex-col gap-2'>
                ...
                {imagesOdd.map((media: Media, index) => (
                    <MediaItem key={index} index={index} src={media.content} type={media.type} likes={media.likes} mediaID={media.media_id} liked={media.liked} section_id={media.section_id} sections={sections} onZoom={() => onZoom(media)}/>
                ))}
            </div>
            <div className='flex flex-col gap-2'>
